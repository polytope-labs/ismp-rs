...
impl host::ISMPHost for HP {
    ...
    fn consensus_state(&self, id: ConsensusClientId) -> Result<Vec<u8>, Error> {
        match self.client.borrow().get(&id).ok_or(Error::ConsensusStateNotFound { id }) {
            Ok(val) => Ok(val.state.to_vec()),
            Err(e) => Err(e)
        }
    }
    ...
    fn store_consensus_state(&self, id: ConsensusClientId, state: Vec<u8>) -> Result<(), Error> {
       match self.client.borrow_mut().get_mut(&id) {
        Some(val) => {
            val.state = state;
            Ok(())
        },
    ...
    fn store_state_machine_update_time(
        ...
        match self.commits.borrow_mut().get_mut(&height) {
            Some(val) => {
    ...
    fn consensus_client(&self, id: ConsensusClientId) -> Result<Box<dyn ConsensusClient>, Error> {
        match self.client.borrow().get(&id).ok_or(Error::ConsensusStateNotFound { id }).cloned() {
